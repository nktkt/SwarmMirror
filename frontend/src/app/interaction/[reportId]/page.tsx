...
export default function InteractionPage() {
  ...
  const statusClass = currentStatus === "error" ? "bg-red-500" : currentStatus === "processing" ? "bg-amber-500 animate-[pulse-dot_1s_infinite]" : "bg-green-500"
  const statusLabel = currentStatus === "error" ? "Error" : currentStatus === "processing" ? "Processing" : "Ready"
    ...
    <div className="h-screen flex flex-col bg-white overflow-hidden">
      <header className="h-[60px] border-b border-gray-200 flex items-center justify-between px-6 bg-white z-[100] relative">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 text-sm"><span className="font-mono font-bold text-gray-400">Step 5/5</span><span className="font-bold">Interaction</span></div>
      ...
      <main className="flex-1 flex overflow-hidden">
        <div className={`h-full overflow-hidden transition-all duration-400 ease-[cubic-bezier(0.25,0.8,0.25,1)] ${leftStyle} border-r border-gray-200`}>
          <GraphPanel graphData={graphData} loading={graphLoading} currentPhase={5} onRefresh={() => projectData?.graph_id && loadGraph(projectData.graph_id)} onToggleMaximize={() => setViewMode(viewMode === "graph" ? "split" : "graph")} />
        </div>
        <div className={`h-full overflow-hidden transition-all duration-400 ease-[cubic-bezier(0.25,0.8,0.25,1)] ${rightStyle}`}>
          <Step5Interaction reportId={reportId} simulationId={simulationId} systemLogs={systemLogs} onAddLog={addLog} onUpdateStatus={setCurrentStatus} />
        </div>
      </main>
    </div>
  )
